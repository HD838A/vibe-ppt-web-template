...
3. 没有真实素材时，继续使用 /placeholder.svg、/placeholder-avatar.svg 或 /placeholder-qr.svg。
4. 不写入未授权的个人信息、真实联系方式、账号密码、API Key、Cookie、Base64 图片或远程图片 URL。
5. 所有数据、数字和引用都必须来自我提供的资料；不确定的内容使用“待补充”而不是编造。
6. 保持 10–14 页的叙事节奏：封面 → 背景 → 案例 → 章节转场 → 方法 → 证据 → 例子 → 路线图 → 反馈 → 能力 → 结论 → 行动邀请。
