...
- 叠卡画廊必须同时保留所有卡片，通过层级、旋转和位移重排主卡；不能只卸载旧图再渲染一张新图。
- 结束页滚动画廊使用两条反向无缝轨道，默认周期分别为 22s 和 28s；轨道素材重复一组形成连续循环。
- 结束页同时配置入口图片和滚动画廊时，左右区域默认各占 50%；入口图片使用 `contain`，桌面高度允许达到 322px，不能通过裁切二维码或海报内容来换取视觉填充。
- 二维码扫描线、封面光斑等连续动画只能用于装饰，不得影响信息读取。
- 必须支持 `prefers-reduced-motion: reduce`：关闭连续动画，把入场和翻页缩短到近似瞬时。

## 演讲模式视口契约

- 滚动浏览可以保持 16:9 幻灯片比例；演讲模式必须明确选择“保留比例留黑边”或“填满视口”。本模板采用填满视口，避免 16:10、超宽屏等设备出现顶部或底部空白。
- 演讲模式下 `.deck`、活动 `.slide-frame` 和 `.slide` 必须使用完整 `100vw × 100vh`，不能继续套用滚动模式的固定 `1280 × 720` 或 `aspect-ratio: 16 / 9`。
- 页面布局必须依赖 Grid、Flex、百分比和主题 token 自适应视口，不能用只在 1280 × 720 下成立的绝对坐标。
- 主题只决定视觉和布局规范，不得自动替换截图、注入说明文案或改变内容语义。暗色主题需要暗色素材时，应由内容显式指定对应资源。
- 发布前至少在 16:9 和 16:10 两种桌面视口检查演讲模式；逐页检查仍以实际目标设备为准。
